  ...
  <section class="content">
    ...
    if (isset($_SESSION['idActa_inscriAlumno'])){
        $idActa_inscriAlumno=$_SESSION['idActa_inscriAlumno'];
           ...
           $consulta = "SELECT `actas_examen_datos_$cicloLectivo`.`idActa`,`actas_examen_datos_$cicloLectivo`.`tipo`, `plan_datos_asignaturas`.`ciclo`, `plan_datos_asignaturas`.`idAsig` ,`plan_datos_asignaturas`.`nombre` AS 'nombreAsignatura', `plan_datos_asignaturas`.`idPlan`, `actas_examen_datos_$cicloLectivo`.`precentacion`, `acta_examen_equipo_pedagogico_$cicloLectivo`.`titulo`, `actas_examen_datos_$cicloLectivo`.`edicion_docente` FROM `actas_examen_datos_$cicloLectivo` INNER JOIN `plan_datos_asignaturas` ON `plan_datos_asignaturas`.`idAsig` = `actas_examen_datos_$cicloLectivo`.`idAsignatura` INNER JOIN `acta_examen_equipo_pedagogico_$cicloLectivo` ON `acta_examen_equipo_pedagogico_$cicloLectivo`.`id` = `actas_examen_datos_$cicloLectivo`.`id_equipo` WHERE `actas_examen_datos_$cicloLectivo`.`idActa` = '$idActa_inscriAlumno'";
                            ...
                            $edicion_docente=$d1at['edicion_docente']; 

                            $disab='disabled';
                            if (($operacion=='Lectura y Escritura') && ($edicion_docente=='ACTIVO')){ 
...
?>
              ...
              <div class="card-body">
                ...
                <?php if (($operacion=='Lectura y Escritura') && ($edicion_docente=='ACTIVO')){ ?>
                       ...
                       <?php  
                           ...
                           $consulta = "SELECT `acta_examen_inscrip_$cicloLectivo`.`idInscripcion`, `datosalumnos`.`idAlumnos`,`datosalumnos`.`nombreAlumnos`, `datosalumnos`.`dniAlumnos`, `acta_examen_inscrip_$cicloLectivo`.`notaEsc`, `acta_examen_inscrip_$cicloLectivo`.`notaOral`, `acta_examen_inscrip_$cicloLectivo`.`promNumérico`, `acta_examen_inscrip_$cicloLectivo`.`promLetra` FROM `acta_examen_inscrip_$cicloLectivo` INNER JOIN `datosalumnos` ON `datosalumnos`.`idAlumnos` = `acta_examen_inscrip_$cicloLectivo`.`idAlumno` WHERE `acta_examen_inscrip_$cicloLectivo`.`idActa` = '$idActa_inscriAlumno'";
                            ...
                            <tr id="<?php echo $idInscripcion ?>" class="table-<?php echo $colorFinal; ?>">
                                ...
                                <td><input type="number" class="form-control bg-dark-x border-0" id="notaEsc_<?php echo $idInscripcion; ?>" value="<?php echo $notaEsc; ?>" <?php echo $disab; ?>></td>

                                <td><input type="number" class="form-control bg-dark-x border-0" id="notaOral_<?php echo $idInscripcion; ?>" value="<?php echo $notaOral; ?>" <?php echo $disab; ?>></td>

                                <td><input type="number" class="form-control bg-dark-x border-0" id="promNumérico_<?php echo $idInscripcion; ?>" value="<?php echo $promNumérico; ?>" <?php echo $disab; ?>></td>

                                <td><input type="text" class="form-control bg-dark-x border-0" id="promLetra_<?php echo $idInscripcion; ?>" value="<?php echo $promLetra; ?>" <?php echo $disab; ?>></td>

                                <td><input type='checkbox' class="seleTod" value="<?php echo $idInscripcion ?>" <?php echo $disab; ?>></input></td>
                            </tr>
                           <?php } ?>                            
                        </tbody>        
                       </table>                    
