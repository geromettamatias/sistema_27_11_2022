        ...
        foreach ($columna as &$Nombrecolum) {
                ...
                foreach($data as $dat) {
                  ...
                  $notaFinal=$dat[''.$Nombrecolum.''];
                   ...
                   $consulta1 = "SELECT `idCabezera`, `nombre`, `descri`, `editarDocente`, `corresponde`, `tipo` FROM `cabezera_libreta_digital_$cicloLectivo` WHERE `nombre`='$Nombrecolum'";
...
$consulta = "SELECT `idDatosFicha`, `idAlumno`, `promovido`, `ob`, `lugarFecha` FROM `datoslibreta_$cicloLectivo` WHERE `idAlumno`= '$idAlumnos'";
                $resultado = $conexion->prepare($consulta);
                $resultado->execute();
                $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
                foreach($data as $dat) {
